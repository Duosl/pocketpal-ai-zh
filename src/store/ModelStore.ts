...
class ModelStore {
  ...
  getEffectiveValues = () => {
    // 确保上下文大小不小于模型要求的最小值
    const minContextSize = Math.max(this.MIN_CONTEXT_SIZE, 2048);
    const effectiveContext = Math.max(this.n_context, minContextSize);

    // 优化批处理大小
    const effectiveBatch = Math.min(
      Math.max(this.n_batch, effectiveContext / 4),
      effectiveContext
    );

    // 优化微批处理大小
    const effectiveUBatch = Math.min(
      Math.max(this.n_ubatch, effectiveBatch / 2),
      effectiveBatch
    );
  ...
  // ModelStore.ts 中的 chatTitle 计算属性
  ...
  get chatTitle() {
      ...
      if (activeModel) {
        ...
        // 如果没有 name 属性，尝试从 metadata 中获取
        const modelMetadata = (this.context?.model as any)?.metadata;
        if (modelMetadata?.['general.name']) {
          return modelMetadata['general.name'];
        }
      }
